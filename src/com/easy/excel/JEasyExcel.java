...
public class JEasyExcel {
	...
	public boolean open(File file)
	{
		...
		try
		{
			...
			} else if (file.exists())
			{
				...
				excelFile = file;
				FileUtils
						.copyFile(excelFile.getAbsolutePath(), excelFile
								.getAbsolutePath().substring(0, excelFile
										.getAbsolutePath().lastIndexOf("/")) + "/copyExcel.xls");
				copyFile = new File(excelFile.getAbsolutePath()
						.substring(0, excelFile.getAbsolutePath()
								.lastIndexOf("/")) + "/copyExcel.xls");
				isFileExists = true;
	...
	public boolean createSheet(String sheetName)
	{
		...
		{
			sheet = workbook
					.createSheet(sheetName, getSheet() == null ? 0 : getSheet().length);// 第一个参数为工作簿的名称，第二个参数为页数
		} catch (Exception e)
		{
			// TODO: handle exception
			e.printStackTrace();
     ...
     * @return
     */
	public boolean writeDate(String sheetName, int col, int row, String value,Colour colour)
	{
		sheet = workbook.getSheet(sheetName);
		// sheet.
		try
	...
	public boolean colseExcel()
	{
		...
		if (isFileExists)
		{
			...
			{
				// 如果删除成功
				FileUtils.copyFile(copyFile.getAbsolutePath(), excelFile
						.getAbsolutePath());
				copyFile.delete();
			}
		}
